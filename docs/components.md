...
*   **Implementation**: Implemented by `IcebergSink` in `crates/storage`, `KafkaSink` in `crates/kafka-sink`, `StarRocksSink` in `crates/starrocks-sink`, and `ElasticsearchSink` in `crates/elasticsearch-sink`.
